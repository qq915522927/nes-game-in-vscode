...
function keyboard(callback, event, stat, type) {
	var localsetkeys = localkeybords;
	if (type == 1) {
		localsetkeys = localGampad;
	}
	if (localsetkeys[event.keyCode]) {
		var h = localsetkeys[event.keyCode][0] - 1;
		var l = localsetkeys[event.keyCode][1] - 2;
		var player = l + 1;
...
function touchInit(type) {
	...
	$("#joystick_btn_menu").bind("touchstart gesturestart touchmove", function(
		e
	) {
	...
	$("#joystick_btn_start").bind("touchstart gesturestart touchmove", function(
		e
	) {
	...
	$("#joystick_btn_choice").bind("touchstart gesturestart touchmove", function(
		e
	) {
			...
			$(lie)
				...
				.append(
					"<img class='avatar' src='" +
						...
						nesInfo.time +
						"</span><button type='button'  onclick='saveAndload(\"1\"," +
						id1 +
						" )' class='right btn btn-success'>读取</button><button type='button' class='right2 btn btn-success' onclick='saveAndload(\"0\",\"" +
						id +
						'",this,' +
						id1 +
						"," +
	...
	dbtool.getNesListCall(id + "info", function(neslist) {
		...
		if (neslist.length < 3) {
			for (var i = neslist.length; i < 3; i++) {
				$("#savelist").append(
					"<li><div class='avatar'></div> <span class='name'>存档[" +
						(i + 1) +
						"]</span> <span class='time'>未存档</span><button type='button' class='right btn btn-success' onclick='saveAndload(\"0\",\"" +
						id +
						"\",this)'>存档</button></li>"
				);
			}
...
function initSavelistPC(gid) {
	...
	$("#cheatscontent").css({
		...
		left:
			$(".tv-screen").offset().left - $("#cheatscontent").width() - 20 + "px"
	});
	$(".tab-pane-bg").css({
		top: $(".tv-content").offset().top - 20 + "px",
...
function gamepadloop() {
	...
	for (let gamepadIndex = 0; gamepadIndex < gamepads.length; gamepadIndex++) {
		...
		var playnumer = getPLaynum(gamepad.id + gamepadIndex);
		if (window.hassetpad && window.issetBtn) {
			...
			for (var k in localGampad) {
				if (k.indexOf("btn") != -1) {
					var btnindex = k.split("-")[1];
					btnindex = btnindex.substring(3);
					const button = buttons[btnindex];
					const previousButton = previousButtons[btnindex];
					if (button.pressed && !previousButton.pressed) {
						keyboard(nes.buttonDown, { keyCode: k }, 1, 1);
					} else if (!button.pressed && previousButton.pressed) {
						keyboard(nes.buttonUp, { keyCode: k }, 0, 1);
					}
				} else if (k.indexOf("axis") != -1) {
					var axistemp = localGampad[k][2].split(" ")[0];
					axisindex = axistemp.substring(4);
					const axis = gamepad.axes[axisindex];
					const previousAxis = previousGamepad.axes[axisindex];
					var axisvalue = localGampad[k][2].split(" ")[1];
					axisvalue = parseInt(axisvalue);
					if (axis === axisvalue && previousAxis !== axisvalue) {
						keyboard(nes.buttonDown, { keyCode: k }, 1, 1);
					}
					if (axis !== axisvalue && previousAxis === axisvalue) {
						keyboard(nes.buttonUp, { keyCode: k }, 0, 1);
...
function startchat(roomid) {
	...
	socket.on("msg", function(userName, msg) {
		...
		var message =
			'<li class="message"><div class="avatar" style="background-image: url(https://yikm.net/avatar/' +
			avatarnum +
			'.png);"></div><div class="speaker">' +
			realuserName +
			'</div><div class="textContainer text-border text-border-left me"><div class="text">' +
			msg +
			"</div></div></li>";
	...
	socket.on("sys", function(sysMsg, type, title) {
		...
		if (realuserName) {
			var message =
				'<li class="message"><span class="subject">' +
				realuserName +
...
function initCheatCon() {
	...
	if (gamecheatList.length > 0) {
		for (var i = 0; i < gamecheatList.length; i++) {
			$("#cheatlist").append(
				'<li><span class="name" style="margin-top: 5px">' +
					gamecheatList[i].desc +
					'</span><label><input type="checkbox" onchange="changecheat(this.checked,' +
					i +
					')" class="ios-switch green  bigswitch" /><div class="cheatswitch"><div></div></div></label></li>'
			);
		}
	}
}
...
function initLocalkeys() {
	...
	if (lstorage.gamepadkeys) {
		localGampad = JSON.parse(lstorage.gamepadkeys);
	} else {
		localGampad = {
			13: [9, 2, ""],
			37: [3, 3, ""],
		...
		lstorage.gamepadkeys = JSON.stringify(localGampad);
	}
}
	...
	for (var k in localkeybords) {
		$(
			"#keytable tr:eq(" +
				...
				localkeybords[k][1] +
				")"
		).html(localkeybords[k][2]);
	}
}
function initGampadKey() {
	$("#keytable tr td:not(:nth-child(1))").html("");
	for (var k in localGampad) {
		$(
			"#keytable tr:eq(" +
				localGampad[k][0] +
				...
				localGampad[k][1] +
				")"
		).html(localGampad[k][2]);
	}
}
function changeGamepadAxis(btnindex, btncode, h, l, value) {
	var btnname = "axis" + btncode + " " + value;
	gamepadid = btnindex + "-" + btnname;
	var currentKey = localGampad[gamepadid];
	if (currentKey) {
		$(
			"#keytable tr:eq(" +
				currentKey[0] +
	...
	$("#keytable tr:eq(" + h + ") td:nth-child(" + l + ")").html(btnname);
	$("#keytable").unbind("keydown");
	for (var k in localGampad) {
		if (localGampad[k][0] == h && localGampad[k][1] == l) {
			delete localGampad[k];
			break;
		}
	}
	localGampad[gamepadid] = [h, l, btnname];
	lstorage.gamepadkeys = JSON.stringify(localGampad);
	window.hassetpad = false;
}
function changeGamepadKey(btnindex, btncode, h, l) {
	var btnname = "btn" + btncode;
	gamepadid = btnindex + "-" + btnname;
	var currentKey = localGampad[gamepadid];
	if (currentKey) {
		$(
